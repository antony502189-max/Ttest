...
class Settings(BaseSettings):
    ...
    external_import_sources: str = "fotocasa,milanuncios,pisocompartido,pisos,alquilerdocentecanarias,flatio"
    ...
    def validate_runtime(self) -> None:
        ...
        if self.max_active_listings_per_user < 1:
            problems.append("Active listing limit must be positive")
        if (
        ...
        if self.is_production:
            ...
            if self.external_import_enabled:
                ...
                if self.external_import_min_healthy_sources > len(configured_sources):
                    problems.append(
                        "EXTERNAL_IMPORT_MIN_HEALTHY_SOURCES cannot exceed configured sources"
                    )
