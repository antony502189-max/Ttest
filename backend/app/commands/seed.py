...
PLACES = (
    ("Adeje", "Costa Adeje", 28.0902, -16.7260), ("Adeje", "Armeñime", 28.1272, -16.7390),
    ("Arona", "Playa de las Américas", 28.0640, -16.7310), ("Arona", "Los Cristianos", 28.0509, -16.7172),
    ("Granadilla de Abona", "San Isidro", 28.0770, -16.5580), ("Granadilla de Abona", "El Médano", 28.0477, -16.5363),
    ("Santa Cruz de Tenerife", "Santa Cruz de Tenerife", 28.4636, -16.2518),
    ("San Cristóbal de La Laguna", "La Laguna", 28.4874, -16.3159), ("Adeje", "Adeje", 28.1227, -16.7244),
    ("Arona", "Arona", 28.0996, -16.6809),
)
TITLES = (
    "Habitación luminosa con escritorio y gastos incluidos", "Habitación doble cerca de la playa y la guagua",
    "Habitación con baño privado para teletrabajo", "Habitación tranquila en piso compartido reformado",
    "Habitación amueblada junto a todos los servicios", "Estudio privado con cocina y terraza",
    "Habitación exterior con armario empotrado", "Habitación para curso universitario junto al tranvía",
    "Habitación amplia con balcón y fibra", "Habitación económica en vivienda organizada",
)
OWNERS = (
    ("Equipo Casa Norte", "CN"), ("Marina A.", "MA"), ("Daniel R.", "DR"), ("Vivienda Campus", "VC"),
    ("Isla Rooms", "IR"), ("Atlántico Estancias", "AE"), ("Nerea S.", "NS"), ("Clara M.", "CM"),
    ("Tenerife Hogar", "TH"), ("Raúl G.", "RG"),
)
...
async def demo_owner(session, index: int) -> User:
    ...
    if owner:
        return owner
    owner = User(
        email=email, name=name, initials=initials, role="host", password_hash=None,
        phone="+34 600 112 233", whatsapp="+34 611 223 344", show_phone=True, show_whatsapp=True,
        allow_contact_form=True, email_verified=index % 7 != 0,
    )
    session.add(owner)
    await session.flush()
    return owner
...
async def import_current_demo_listings(session) -> None:
    for index in range(32):
        ...
        tenant_requirement = "single-woman" if index % 5 == 0 else "single-man" if index % 7 == 0 else "couple" if index % 3 == 0 else "any"
        ...
        session.add(Listing(
            id=listing_id, owner_user_id=owner.id, title=TITLES[index % len(TITLES)], city=city, area=area,
            street="", postcode="", approximate_address=("Zona centro", "Cerca de la plaza", "A 8 min de la costa", "Junto a la parada principal")[index % 4] + " · ubicación aproximada",
            rental_mode=rental_mode, monthly_price=price if rental_mode == "long" else price * 24,
            nightly_price=price if rental_mode == "holiday" else None, weekly_price=price * 6 if rental_mode == "holiday" else None,
            room_type="Estudio" if index % 9 == 5 else "Habitación compartida" if index % 8 == 3 else "Habitación individual",
            ...
            deposit_amount=0 if index % 6 == 0 else price, bills_included=bills_included,
            bathroom="Baño privado" if index % 4 == 2 else "Baño compartido",
            kitchen="Cocina privada" if index % 9 == 5 else "Cocina compartida", furnished=index % 11 != 0,
            room_size_m2=9 + (index % 10), bedroom_count=bedroom_count, current_residents=1 + (index % 6),
            room_capacity=room_capacity, shower="Ducha privada" if index % 4 == 2 else "Ducha compartida",
            tenant_requirement=tenant_requirement, smoking_allowed=index % 6 == 0, pets_allowed=index % 4 == 0,
            children_allowed=index % 6 == 1, empadronamiento_allowed=index % 2 == 0,
            restrictions=[f"Requisito: {tenant_requirement}", "Gastos incluidos" if bills_included else "Gastos aparte"],
            ...
            status="published", location=ST_SetSRID(ST_MakePoint(longitude + ((index % 4) - 1.5) * 0.004, latitude + ((index % 3) - 1) * 0.0045), 4326),
            ...
            advertiser_type="Profesional" if index % 4 == 0 else "Particular", source="Anunciante profesional" if index % 4 == 0 else None,
            published_at=datetime(2026, 7, max(1, 20 - (index % 20)), 12 - (index % 8), tzinfo=UTC),
            expires_at=datetime(2026, 10, 1 + (index % 27), tzinfo=UTC), views=90 + index * 37,
        ))
