...
async def stats(user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
...
async def get_user_route(user_id: UUID, user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
...
async def unrestrict_user_route(user_id: UUID, user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
    await _lock_user_mutation(user_id, session)
...
async def list_notes_route(user_id: UUID, user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
...
async def list_admins_route(user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
...
async def add_admin_route(payload: AddAdminRequest, user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
...
async def revoke_admin_route(email: str, user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
    await _lock_admin_access(session)
...
async def external_import_runs(
    ...
    rows = (
        await session.scalars(
            select(ExternalImportRun).order_by(ExternalImportRun.started_at.desc()).limit(limit).offset(offset)
        )
    ).all()
    return [
        ExternalImportRunResponse(
...
async def external_import_worker_state(user: User = Depends(require_admin), session: AsyncSession = Depends(get_session)):
    ...
    heartbeat_deadline = datetime.now(UTC) - timedelta(seconds=get_settings().external_worker_stale_after_seconds)
    health = (
        "delayed"
