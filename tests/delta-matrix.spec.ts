...
test('LOC-01 selected zone coordinates persist, edit restores them and exact street stays private', async ({ page }) => {
  ...
  await map.dblclick({ position: { x: Math.round((box?.width ?? 300) * 0.68), y: Math.round((box?.height ?? 240) * 0.42) } })
  await expect.poll(async () => output.textContent()).not.toBe(before)

  const movedText = await output.textContent()
  const movedMatch = movedText?.match(/(-?\d+\.\d+),\s*(-?\d+\.\d+)/)
  expect(movedMatch).not.toBeNull()
  const movedLat = Number(movedMatch?.[1])
  const movedLng = Number(movedMatch?.[2])
