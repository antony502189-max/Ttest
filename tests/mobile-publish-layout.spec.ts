...
for (const width of [320, 360, 375, 390, 430]) {
  test(`mobile publish header and location controls stay inside ${width}px viewport`, async ({ page }) => {
    ...
    const header = page.locator('.listing-edit-topbar')
    const back = header.getByRole('link', { name: 'Tus anuncios' })
    ...
    expect(backBox!.x + backBox!.width).toBeLessThanOrEqual(width + 1)
