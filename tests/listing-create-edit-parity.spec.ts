...
test('create and edit keep separate persistence and mutation paths', () => {
  ...
  expect(createFlow).toContain('prepared = await prepareListingMedia(optimistic.images, optimistic.video)')
  expect(createFlow).toContain('createRemoteListing(optimistic, prepared.images.assetIds, prepared.video.assetId)')
  expect(updateFlow).toContain('prepared = await prepareListingMedia(next.images, next.video)')
  expect(updateFlow).toContain('updateRemoteListing(id, next, prepared.images.assetIds, prepared.video.assetId, previous)')
