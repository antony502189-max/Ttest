...
test('customer video fix keeps owner routes behind authoritative hydration', () => {
  ...
  expect(context).toContain("export type OwnedListingsHydrationStatus = 'idle' | 'loading' | 'ready' | 'error'")
  expect(context).toContain("setOwnedListingsHydrationStatus('ready')")
  expect(context).toContain('refreshOwnedListings: refreshOwnedConsumers')
  expect(gate).toContain("app.ownedListingsHydrationStatus !== 'ready'")
  expect(gate).toContain('app.refreshOwnedListings()')
  expect(gate).not.toContain('getOwnedListings(')
  expect(gate).not.toContain("catalog:updated")
