...
test('customer video fix keeps owner routes behind authoritative hydration', () => {
  ...
  expect(gate).toContain('PROVIDER_SYNC_RETRY_MS')
...
test('customer video fix keeps fresh location unresolved safely and validates it at the wizard model boundary', () => {
  ...
  expect(publish).toContain('!mockMode && (draft.street.trim() || draft.postcode.trim()) && confirmedAddressRef.current !== listingAddressFingerprint(draft)')
...
test('customer video: listing edit prepares media first and commits fields plus image order atomically', () => {
  const context = readFileSync('src/contexts/app-context.tsx', 'utf8')
  const media = readFileSync('src/api/media.ts', 'utf8')
  const api = readFileSync('src/api/listings.ts', 'utf8')
  const backend = readFileSync('backend/app/services/listings.py', 'utf8')
  const schema = readFileSync('backend/app/schemas/listings.py', 'utf8')
  const start = context.indexOf('const updateListing = useCallback(async')
  const end = context.indexOf('const deleteListing', start)
...
test('customer video: edit rejects a different municipality as area while production requires structured address confirmation', async ({ page }) => {
  const editSource = readFileSync('src/pages/ListingEditPage.tsx', 'utf8')
  expect(editSource).toContain("!mockMode && (draft.street.trim() || draft.postcode.trim()) && confirmedAddressRef.current !== listingAddressFingerprint(draft)")
  ...
  await page.getByLabel('Zona o barrio').fill('Arona')
  await page.getByRole('button', { name: 'Guardar cambios' }).click()
  await expect(page.getByText(/corresponde a otro municipio/i)).toBeVisible()
})
