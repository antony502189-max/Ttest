...
for (const viewport of mobileViewports) {
  test(`mobile home rental cards stay aligned after selection at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    expect(before[0].width - before[1].width).toBeGreaterThanOrEqual(6)
    expect(before[0].width - before[1].width).toBeLessThanOrEqual(10)
    expect(before[0].x).toBeGreaterThanOrEqual(6)
    expect(before[1].x + before[1].width).toBeLessThanOrEqual(viewport.width - 18)
    ...
    expect(after[1].x + after[1].width).toBeLessThanOrEqual(viewport.width - 18)
