...
function FilterPanel({ value, onChange, rentalMode }: { value: Filters; onChange: (value: Filters) => void; rentalMode: RentalMode }) {
  ...
  return <div className="filter-panel">
    ...
    <section className="filter-section"><h3>{text("Habitación", "Комната", "Room")}</h3>
      ...
      <div className="form-grid form-grid--compact"><label className="field-label">{text("Habitación desde (m²)", "Площадь комнаты от (м²)", "Room size from (m²)")}<NumericInput ariaLabel={text("Tamaño mínimo (m²)", "Минимальная площадь (м²)", "Minimum size (m²)")} min={0} max={100} value={value.roomSizeMin} onValueChange={(next) => onChange({ ...value, roomSizeMin: Math.min(next, value.roomSizeMax) })} /></label><label className="field-label">{text("Habitación hasta (m²)", "Площадь комнаты до (м²)", "Room size to (m²)")}<NumericInput ariaLabel={text("Tamaño máximo (m²)", "Максимальная площадь (м²)", "Maximum size (m²)")} min={1} max={100} value={value.roomSizeMax} onValueChange={(next) => onChange({ ...value, roomSizeMax: Math.max(next, value.roomSizeMin, 1) })} /></label></div>
      <NativeSelect label={text("Tipo de cama", "Тип кровати", "Bed type")} value={value.bedType} options={[{ value: "Cualquiera", label: anyLabel }, { value: "single", label: text("Individual", "Односпальная", "Single") }, { value: "double", label: text("Doble", "Двуспальная", "Double") }]} onChange={(next) => update("bedType", next as Filters["bedType"])} />
    ...
    <section className="filter-section"><h3>{text("Espacios y equipamiento", "Помещения и оснащение", "Spaces and equipment")}</h3>
      ...
      <NativeSelect label={text("Cocina / mini-cocina en la habitación", "Личная кухня / мини-кухня в комнате", "Kitchen / kitchenette in the room")} value={value.kitchen} options={[{ value: "Cualquiera", label: anyLabel }, { value: "Cocina privada", label: text("Cocina privada", "Собственная кухня", "Private kitchen") }, { value: "Cocina compartida", label: text("Cocina compartida", "Общая кухня", "Shared kitchen") }]} onChange={(next) => update("kitchen", next)} />
