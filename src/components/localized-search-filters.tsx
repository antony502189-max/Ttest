...
function FilterPanel({ value, onChange, rentalMode }: { value: Filters; onChange: (value: Filters) => void; rentalMode: RentalMode }) {
  ...
  const capacityOptions: SelectOption[] = [
    ...
    ...Array.from({ length: 10 }, (_, index) => ({
      ...
      label: text(`${index + 1} ${index === 0 ? "persona" : "personas"}`, `${index + 1} чел.`, `${index + 1} ${index === 0 ? "person" : "people"}`),
    })),
  ];
  ...
  return (
    <div className="filter-panel">
      <label className="field-label filter-room-only">
        ...
        <select aria-label={text("Tipo de propiedad", "Тип объекта", "Property type")} value="Habitaciones" disabled>
          <option>{text("Habitaciones", "Комнаты", "Rooms")}</option>
        </select>
      </label>

      <section className="filter-section">
        <h3>{rentalMode === "holiday" ? text("Precio por noche", "Цена за ночь", "Price per night") : text("Precio por mes", "Цена за месяц", "Price per month")}</h3>
        <div className="filter-price-fields">
          <label>{text("Desde", "От", "From")}<NumericInput ariaLabel={text("Precio mínimo", "Минимальная цена", "Minimum price")} min={0} max={maxPrice} step={rentalMode === "holiday" ? 5 : 25} value={priceControls.minimum} onValueChange={(next) => onChange({ ...value, minPrice: Math.min(next, priceControls.maximum) })} /></label>
          <label>{text("Hasta", "До", "To")}<NumericInput ariaLabel={text("Precio máximo", "Максимальная цена", "Maximum price")} min={0} max={maxPrice} step={rentalMode === "holiday" ? 5 : 25} value={priceControls.maximum} onValueChange={(next) => onChange({ ...value, minPrice: Math.min(priceControls.minimum, next), maxPrice: Math.max(next, priceControls.minimum) })} /></label>
        </div>
      ...
      <fieldset className="filter-section">
        ...
        <div className="checks-grid">{areas.map((area) => <CheckOption key={area} label={area} checked={value.areas.includes(area)} onCheckedChange={(checked) => update("areas", checked ? [...value.areas, area] : value.areas.filter((item) => item !== area))} />)}</div>
      </fieldset>

      <Separator />
      <section className="filter-section">
        <h3>{text("Habitación y plazas", "Комната и места", "Room and spaces")}</h3>
        ...
        <NativeSelect label={text("Qué se alquila", "Что сдаётся", "Rental unit")} value={value.rentalUnit} options={[
          { value: "Cualquiera", label: anyLabel },
          { value: "room", label: text("Habitación completa", "Комната целиком", "Whole room") },
          { value: "bed", label: text("Plaza / cama", "Спальное место", "Bed space") },
        ]} onChange={(next) => update("rentalUnit", next as Filters["rentalUnit"])} />
        <div className="form-grid form-grid--compact">
          <label className="field-label">{text("Habitación desde (m²)", "Площадь комнаты от (м²)", "Room size from (m²)")}<NumericInput ariaLabel={text("Tamaño mínimo de habitación", "Минимальная площадь комнаты", "Minimum room size")} min={0} max={100} value={value.roomSizeMin} onValueChange={(next) => onChange({ ...value, roomSizeMin: Math.min(next, value.roomSizeMax) })} /></label>
          <label className="field-label">{text("Habitación hasta (m²)", "Площадь комнаты до (м²)", "Room size to (m²)")}<NumericInput ariaLabel={text("Tamaño máximo de habitación", "Максимальная площадь комнаты", "Maximum room size")} min={1} max={100} value={value.roomSizeMax} onValueChange={(next) => onChange({ ...value, roomSizeMax: Math.max(next, value.roomSizeMin, 1) })} /></label>
        </div>
        <NativeSelect label={text("Tipo de cama", "Тип кровати", "Bed type")} value={value.bedType} options={[
          { value: "Cualquiera", label: anyLabel },
          { value: "single", label: text("Individual", "Односпальная", "Single") },
          { value: "double", label: text("Doble", "Двуспальная", "Double") },
        ]} onChange={(next) => update("bedType", next as Filters["bedType"])} />
      ...
      <section className="filter-section">
        ...
        <label className="field-label">{text("Necesito entrar desde", "Нужно заселиться с", "Need from")}<Input type="date" value={value.available} onChange={(event) => update("available", event.target.value)} /></label>
        <label className="field-label">{text("Necesito alojamiento hasta", "Нужно жильё до", "Need until")}<Input type="date" value={value.availableUntil} onChange={(event) => update("availableUntil", event.target.value)} /></label>
        {rentalMode === "long" ? <NativeSelect label={text("Estancia mínima aceptada", "Допустимый минимальный срок", "Accepted minimum stay")} value={value.minStay} options={[
          { value: "Cualquiera", label: anyLabel }, { value: "1", label: text("1 mes", "1 месяц", "1 month") }, { value: "2", label: text("2 meses", "2 месяца", "2 months") }, { value: "3", label: text("3 meses", "3 месяца", "3 months") }, { value: "6", label: text("6 meses", "6 месяцев", "6 months") }, { value: "12", label: text("12 meses", "12 месяцев", "12 months") },
        ]} onChange={(next) => update("minStay", next)} /> : <label className="field-label">{text("Estancia mínima: hasta (noches)", "Минимальный срок — не более (ночей)", "Minimum stay up to (nights)")}<NumericInput ariaLabel={text("Estancia mínima", "Минимальный срок", "Minimum stay")} min={0} max={365} value={value.minimumNights} onValueChange={(next) => update("minimumNights", next)} /></label>}
        <NativeSelect label={text("Publicado", "Опубликовано", "Published")} value={value.publicationDate} options={[
          { value: "Cualquiera", label: anyLabel }, { value: "24h", label: text("Últimas 24 horas", "За последние 24 часа", "Last 24 hours") }, { value: "7d", label: text("Últimos 7 días", "За последние 7 дней", "Last 7 days") }, { value: "30d", label: text("Últimos 30 días", "За последние 30 дней", "Last 30 days") },
        ]} onChange={(next) => update("publicationDate", next)} />
      </section>

      <Separator />
      <section className="filter-section">
        <h3>{text("Vivienda y convivencia", "Квартира и жильцы", "Home and household")}</h3>
        <div className="form-grid form-grid--compact">
          <label className="field-label">{text("Vivienda desde (m²)", "Площадь жилья от (м²)", "Home size from (m²)")}<NumericInput ariaLabel={text("Vivienda desde", "Площадь жилья от", "Home size from")} min={0} max={500} value={value.homeSizeMin} onValueChange={(next) => onChange({ ...value, homeSizeMin: Math.min(next, value.homeSizeMax) })} /></label>
          <label className="field-label">{text("Vivienda hasta (m²)", "Площадь жилья до (м²)", "Home size to (m²)")}<NumericInput ariaLabel={text("Vivienda hasta", "Площадь жилья до", "Home size to")} min={1} max={500} value={value.homeSizeMax} onValueChange={(next) => onChange({ ...value, homeSizeMax: Math.max(next, value.homeSizeMin, 1) })} /></label>
        </div>
        ...
        <NativeSelect label={text("Residentes actuales en la vivienda", "Сейчас живут в квартире", "Current home residents")} value={value.currentResidents} options={[{ value: "Cualquiera", label: anyLabel }, "0", "1", "2", "3", "4", { value: "5+", label: text("5 o más", "5 и более", "5 or more") }]} onChange={(next) => update("currentResidents", next)} />
        <NativeSelect label={text("Composición de la vivienda", "Состав жильцов", "Household composition")} value={value.householdGender} options={[
          { value: "Cualquiera", label: anyLabel }, { value: "men", label: text("Hombres", "Мужчины", "Men") }, { value: "women", label: text("Mujeres", "Женщины", "Women") }, { value: "mixed", label: text("Mixto", "Совместное проживание", "Mixed") },
        ]} onChange={(next) => update("householdGender", next as Filters["householdGender"])} />
      ...
      <section className="filter-section">
        ...
        <div className="checks-grid">{amenityOptions.map((amenity) => <CheckOption key={amenity} label={t(amenity)} checked={value.amenities.includes(amenity)} onCheckedChange={(checked) => update("amenities", checked ? [...value.amenities, amenity] : value.amenities.filter((item) => item !== amenity))} />)}</div>
      </section>

      <Separator />
      <section className="filter-section">
        <h3>{text("Quién puede alojarse", "Кого принимают", "Who can stay")}</h3>
        <NativeSelect label={text("Condición principal", "Основное требование", "Primary requirement")} value={value.tenantRequirement} options={tenantRequirements} onChange={(next) => update("tenantRequirement", next as Filters["tenantRequirement"])} />
        ...
        <YesNoFilter label={text("Menores / niños permitidos", "Можно с детьми", "Children allowed")} value={value.children} onChange={(next) => update("children", next)} language={language} />
      ...
      <Separator />
      <section className="filter-section">
        <h3>{text("Fianza y anunciante", "Депозит и арендодатель", "Deposit and advertiser")}</h3>
...
export function FilterButton({ resultCount, onFiltersChange, onRentalModeChange }: { resultCount: number; onFiltersChange?: (filters: Filters) => void; onRentalModeChange?: (mode: RentalMode) => void }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild><Button variant="outline" aria-label={text(`Todos los filtros. ${resultCount} habitaciones actuales`, `Все фильтры. Сейчас комнат: ${resultCount}`, `All filters. ${resultCount} rooms currently`)}><SlidersHorizontal data-icon="inline-start" />{text("Filtros", "Фильтры", "Filters")}{activeFilterCount ? <span className="filter-count">{activeFilterCount}</span> : null}</Button></SheetTrigger>
      <SheetContent className="filter-drawer" showCloseButton={false}>
        <SheetHeader>
          <SheetClose asChild><Button type="button" variant="ghost" size="icon" className="filter-drawer__back" aria-label={text("Cerrar filtros", "Закрыть фильтры", "Close filters")}><X /></Button></SheetClose>
          <SheetTitle>{text("Filtros", "Фильтры", "Filters")}</SheetTitle>
          <SheetDescription>{text("Filtra por habitación, plazas, vivienda, convivencia y disponibilidad.", "Фильтруйте по комнате, местам, квартире, жильцам и датам.", "Filter by room, spaces, home, household and availability.")}</SheetDescription>
        </SheetHeader>
        {onRentalModeChange ? <div className="filter-mode-switch"><span>{text("Tipo de estancia", "Тип аренды", "Stay type")}</span><RentalTypeSwitch compact onChange={onRentalModeChange} /></div> : null}
        <FilterPanel value={draft} onChange={setDraft} rentalMode={rentalMode} />
        <SheetFooter className="filter-footer">
          <Button variant="ghost" onClick={() => setDraft({ ...defaultFilters, areas: [], conditions: [], tenantRequirements: [], acceptedTenantTypes: [], amenities: [] })}>{text("Limpiar", "Сбросить", "Clear")}</Button>
          <Button onClick={() => { commit(draft); setOpen(false); }}>{text(`Mostrar ${draftResultCount} habitaciones`, `Показать: ${roomCount(language, draftResultCount)}`, `Show ${roomCount(language, draftResultCount)}`)}</Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
  );
...
export function FilterSidebar({ resultCount, onFiltersChange }: { resultCount: number; onFiltersChange?: (filters: Filters) => void }) {
  ...
  return (
    <aside className="filter-sidebar" aria-label={text("Filtros de búsqueda", "Фильтры поиска", "Search filters")}>
      <div className="filter-sidebar__save"><Button className="w-full" onClick={saveCurrentSearch}><Heart data-icon="inline-start" />{text("Guardar búsqueda", "Сохранить поиск", "Save search")}</Button><p>{text("Recibe avisos cuando haya habitaciones nuevas que encajen.", "Получайте уведомления о новых подходящих комнатах.", "Get alerts when matching rooms appear.")}</p></div>
      <div className="filter-sidebar__head"><h2>{text("Filtrar resultados", "Фильтровать результаты", "Filter results")}</h2>{activeFilterCount ? <button type="button" onClick={() => commit({ ...defaultFilters, areas: [], conditions: [], tenantRequirements: [], acceptedTenantTypes: [], amenities: [] })}>{text(`Borrar (${activeFilterCount})`, `Сбросить (${activeFilterCount})`, `Clear (${activeFilterCount})`)}</button> : null}</div>
      <FilterPanel value={filters} onChange={commit} rentalMode={rentalMode} />
      <div className="filter-sidebar__result" aria-live="polite">{roomCount(language, resultCount)}</div>
    </aside>
  );
}
