...
const copy = {
  es: {
    ...
    searchOnMap: 'Buscar en el mapa', mapDrawTitle: 'Tu propia zona', visibleArea: 'Zona visible', filters: 'Filtros', list: 'Listado',
  ...
  en: {
    ...
    drawTooShort: 'Draw a larger area in one continuous movement', areaReady: 'Area selected', searchOnMap: 'Search on the map', mapDrawTitle: 'Your own area',
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        const mapId = import.meta.env.VITE_GOOGLE_MAPS_MAP_ID
        const map = new GoogleMap(containerRef.current, { center: TENERIFE_CENTER, zoom: query ? 12 : 10, mapId: mapId || undefined, styles: mapId ? undefined : darkMapStyles, disableDefaultUI: true, gestureHandling: 'greedy', clickableIcons: false, backgroundColor: '#142536', minZoom: 8, maxZoom: 19, restriction: { latLngBounds: { north: 29.2, south: 27.1, east: -15.3, west: -18.2 }, strictBounds: false } })
        mapRef.current = map
...
function MapScreen({ mode, language, t, query, polygon, onPolygonChange, onBack }: {
  mode: MapMode; language: AppLanguage; t: MobileCopy; query: string; polygon: MapPoint[]; onPolygonChange: (polygon: MapPoint[]) => void; onBack: () => void
}) {
...
export function MobileAppV2() {
  ...
  const [mapPolygon, setMapPolygon] = useState<MapPoint[]>([])
  ...
  if (page === 'map') return <div className="m2-app notranslate" translate="no"><MapScreen mode={mapMode} language={language} t={t} query={mapQuery} polygon={mapPolygon} onPolygonChange={setMapPolygon} onBack={() => setPage('location')} /></div>
