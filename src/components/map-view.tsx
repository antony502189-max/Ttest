...
export function LeafletMapView({ items, selectedId, onSelect, fullScreen = false, showPreview = true, onBoundsSearch, onPolygonSearch }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    setError('')
    const map = L.map(containerRef.current, { zoomControl: true, preferCanvas: true, minZoom: 8, zoomAnimation: false, fadeAnimation: false, markerZoomAnimation: false }).setView(tenerifeCenter, 9)
    mapRef.current = map
    ...
    items.forEach((listing) => {
      const marker = L.marker([listing.coordinates.lat, listing.coordinates.lng], { icon: markerIcon(listing), keyboard: true, title: `${listing.area}, ${priceLabel(listing)}` })
    ...
    if (items.length) {
      const points = items.map((item) => L.latLng(item.coordinates.lat, item.coordinates.lng))
      map.fitBounds(L.latLngBounds(points), { padding: [34, 34], maxZoom: 12, animate: false })
    }
    const updateBounds = () => {
  ...
  useEffect(() => {
    ...
    if (marker && mapRef.current) marker.setZIndexOffset(1000)
  }, [items, selectedId])
  ...
  useEffect(() => {
    ...
    if (draftPolygon.length >= 2) {
      polygonLayerRef.current = L.polygon(draftPolygon.map((point) => [point.lat, point.lng]), { color: '#006b72', fillColor: '#d7f20b', fillOpacity: 0.22, weight: 3 }).addTo(map)
    }
  }, [draftPolygon])
  ...
  const addKeyboardPoint = () => {
    ...
    const offsets = [[-.08, -.1], [.07, -.08], [.08, .1], [-.06, .11]]
  ...
  const useCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => mapRef.current?.setView([coords.latitude, coords.longitude], 13),
  ...
  return (
    <section className={cn('leaflet-map-shell', fullScreen && 'leaflet-map-shell--fullscreen')} aria-label="Mapa de habitaciones">
      ...
      {fullScreen ? <div className="map-search-tools" aria-label="Herramientas de búsqueda en mapa">
        {onBoundsSearch ? <Button onClick={searchBounds}><Search data-icon="inline-start" />Buscar en esta zona</Button> : null}
        <Button variant="outline" size="icon" onClick={useCurrentLocation} aria-label="Usar mi ubicación"><Crosshair /></Button>
        {drawing ? <><Button variant="outline" onClick={addKeyboardPoint}><MapPin data-icon="inline-start" />Añadir punto</Button><Button variant="outline" onClick={cancelDrawing}><X data-icon="inline-start" />Cancelar</Button><Button disabled={draftPolygon.length < 3} onClick={finishDrawing}><Check data-icon="inline-start" />Finalizar ({draftPolygon.length})</Button></> : mapPolygon.length >= 3 ? <><Button variant="outline" onClick={() => { saveCurrentSearch(); toast.success('Zona añadida a la búsqueda guardada') }}><Heart data-icon="inline-start" />Guardar zona</Button><Button variant="outline" onClick={deletePolygon}><Trash2 data-icon="inline-start" />Eliminar zona</Button></> : <Button variant="outline" onClick={startDrawing}><Pencil data-icon="inline-start" />Dibujar zona</Button>}
      </div> : null}
      {selected && showPreview ? <article className="map-selected-card" aria-label={`Habitación seleccionada en ${selected.area}`}>
        <img src={selected.images[0]} alt="" width="176" height="120" />
        <div><span>{selected.area}</span><strong>{priceLabel(selected)}/{selected.cadence}</strong><Link to={`/habitacion/${selected.id}`}>{selected.title}</Link></div>
      ...
      <div className="map-list-alternative" aria-label="Alternativa textual al mapa">
        {items.map((item) => <button key={item.id} type="button" onFocus={() => onSelect(item.id)} onMouseEnter={() => onSelect(item.id)} onClick={() => onSelect(item.id)} aria-pressed={item.id === selectedId}><MapPin aria-hidden="true" /><span><strong>{item.area}</strong><small>{priceLabel(item)}/{item.cadence}</small></span></button>)}
      </div>
    </section>
  )
}
