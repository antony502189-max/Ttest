...
export function IdealistaMobileApp() {
  ...
  if (step !== 'done') return <div className="idealista-mobile-app"><Onboarding step={step} setStep={setStep} language={language} setLanguage={setLanguage} /></div>
  ...
  return <div className="idealista-mobile-app">
    <main className="im-main">
      ...
      {tab === 'menu' ? <MenuScreen onLogin={() => setStepState('auth')} onReset={() => { try { localStorage.removeItem(ONBOARDING_KEY) } catch { /* no-op */ }; setStepState('language') }} language={language} t={t} /> : null}
    </main>
