    ...
    <main className="listing-edit-page listing-create-page">
      ...
      <div className="listing-edit-shell">
        ...
        <div className="listing-create-draft-actions">
          ...
          <div>
            <ConfirmDialog trigger={<Button variant="outline" disabled={recoveringImages || processingImages || processingVideo}><RotateCcw data-icon="inline-start" />Restablecer</Button>} title="¿Restablecer el borrador?" description="Se eliminarán los cambios del anuncio y las fotos temporales que no usa ningún anuncio guardado." confirmLabel="Restablecer" destructive onConfirm={resetDraft} />
            <Button variant="outline" disabled={recoveringImages || processingImages || processingVideo || !isDirty} onClick={saveDraft}><Save data-icon="inline-start" />Guardar borrador</Button>
          </div>
        </div>
      ...
      <div className="listing-edit-final"><div><strong>{processingImages ? 'Procesando la foto…' : recoveringImages ? 'Falta terminar las fotografías' : isDirty ? 'El borrador tiene cambios' : 'Borrador guardado automáticamente'}</strong><span>{processingImages ? 'El giro ya se muestra; terminamos de guardar la imagen.' : processingVideo ? 'Terminamos de preparar el vídeo antes de publicar.' : 'Revisamos todos los campos antes de publicar.'}</span></div><Button size="lg" onClick={save} disabled={saving || processingImages || processingVideo}><Save data-icon="inline-start" />{saving ? 'Publicando…' : (processingImages || processingVideo) ? 'Procesando multimedia…' : recoveringImages ? 'Reintentar fotografías' : 'Publicar anuncio'}</Button></div>
    </div>
    </main>
