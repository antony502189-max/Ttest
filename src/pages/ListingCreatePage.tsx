    ...
    <main className="listing-edit-page listing-create-page">
      ...
      <Section disabled={recoveringImages} id="publish-room" title="Habitación y vivienda">
        ...
        <div className="listing-edit-grid listing-edit-grid--3">
          ...
          <FormField label="Capacidad habitación" htmlFor="publish-capacity" error={errors.roomCapacity}><select id="publish-capacity" value={draft.roomCapacity} aria-invalid={Boolean(errors.roomCapacity)} onChange={(e) => set('roomCapacity', Number(e.target.value))}>{Array.from({ length: 10 }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></FormField>
          ...
          <FormField label="Lavadora" htmlFor="publish-washing" error={errors.washingMachine}><select id="publish-washing" value={equipment.washingMachine} aria-invalid={Boolean(errors.washingMachine)} onChange={(e) => setEquipment('washingMachine', e.target.value as EquipmentSelections['washingMachine'])}><option value="">Selecciona</option><option value="individual">Individual</option><option value="shared">Compartida</option><option value="none">No disponible</option></select></FormField>
        </div>
