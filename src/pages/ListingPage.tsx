      ...
      <div className="container listing-layout">
        <div className="listing-main">
          ...
          <section className="listing-comments" aria-labelledby="listing-comments-title">
            ...
            {listingComments.length ? <div className="local-listing-comments" aria-label="Tus comentarios locales">{listingComments.map((comment) => <article key={comment.id}><p>{comment.text}</p><footer><span><span>Guardado en este dispositivo</span> · <time dateTime={comment.updatedAt ?? comment.createdAt}>{new Intl.DateTimeFormat(currentLocale(), { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(comment.updatedAt ?? comment.createdAt))}</time>{comment.updatedAt ? <span> · editado</span> : null}</span><div><Button variant="ghost" size="sm" onClick={() => { setEditingCommentId(comment.id); setCommentText(comment.text); setCommentEditorOpen(true) }}><Pencil data-icon="inline-start" />Editar</Button><Button variant="ghost" size="sm" onClick={() => deleteLocalComment(comment.id)}><Trash2 data-icon="inline-start" />Eliminar</Button></div></footer></article>)}</div> : null}
          </section>
