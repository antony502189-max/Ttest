...
resource_pressure() {
  ...
  awk -v load="$load" -v cpu="$cpu" -v ram="$free_pct" -v disk="$disk_free" \
    'BEGIN { exit !(load >= cpu || ram < 15 || disk < 10) }'
}

light_check() {
  local dir="$1" domain sha http backend worker services load ram disk health line
  domain="$(env_value APP_DOMAIN)"
  [[ "$domain" =~ ^[A-Za-z0-9.-]+$ ]] || domain="invalid"
  ...
  line="$(printf '%s | HEALTH=%s | HTTP=%s | BACKEND=%s | WORKER=%s | SERVICES=%s | DISK=%s | RAM=%s%% | LOAD=%s | SHA=%s' \
    "$(utc_now)" "$health" "${http:-000}" "$backend" "$worker" "$services" "$disk" "$ram" "$load" "$sha")"
  printf '%s\n' "$line" >> "$dir/live.log"
  [[ "$health" == OK ]] || printf '%s\n' "$line" >> "$dir/errors.log"
  [[ "$health" == OK ]]
}
...
capture_import_snapshot() {
  ...
  worker_row="$(timeout 30 "${compose[@]}" exec -T postgres psql -v ON_ERROR_STOP=1 -U "$postgres_user" -d "$postgres_db" -At -F '|' -c "
    SELECT
      COALESCE(health, ''),
      COALESCE(heartbeat_at::text, ''),
      COALESCE(last_started_at::text, ''),
      COALESCE(last_finished_at::text, ''),
      COALESCE(last_success_at::text, ''),
      COALESCE(last_run_id, '')
  ...
  timeout 30 "${compose[@]}" exec -T postgres psql \
    ...
    -U "$postgres_user" -d "$postgres_db" -At -F '|' -c "
      SELECT
        ...
        source_name,
        result,
        COALESCE(discovery_complete::text, ''),
        COALESCE(discovery_pages::text, ''),
        COALESCE(jsonb_array_length(discovery_failed_pages), 0)::text,
        COALESCE(counters->>'discovered_urls', '0'),
        COALESCE(counters->>'fetched_details', '0'),
        COALESCE(counters->>'accepted_rooms', '0')
...
full_check() {
  ...
  if command -v ionice >/dev/null; then
    nice -n 10 ionice -c2 -n7 timeout --foreground "$FULL_TIMEOUT_SECONDS" \
      bash "$ROOT/current/deploy/production-monitor-check.sh" >"$tmp" 2>&1
  else
    nice -n 10 timeout --foreground "$FULL_TIMEOUT_SECONDS" \
      bash "$ROOT/current/deploy/production-monitor-check.sh" >"$tmp" 2>&1
  fi
  rc=$?
  set -e
...

  {
    ...
    kv RELEASE_SHA "$sha"
    kv MONITOR_RETURN_CODE "$rc"
    kv RESULT "$result"
    sanitize < "$tmp"
    printf '\nIMPORT_SNAPSHOT_BEGIN\n'
    set +e
    capture_import_snapshot | sanitize
    snapshot_rc=${PIPESTATUS[0]}
    set -e
    kv IMPORT_SNAPSHOT_RETURN_CODE "$snapshot_rc"
    printf 'IMPORT_SNAPSHOT_END\n'
  } > "$sample"
  rm -f "$tmp"

  snapshot_rc="$(sed -n 's/^IMPORT_SNAPSHOT_RETURN_CODE=//p' "$sample" | tail -n 1)"
  if [[ "$result" != MAINTENANCE && "$snapshot_rc" != 0 ]]; then
    result=FAIL
    sed -i 's/^RESULT=.*/RESULT=FAIL/' "$sample"
  fi
...
make_report() {
  local dir="$1" start end expected sha current final total_full full_pass full_warn full_fail skipped_pressure skipped_running maintenance snapshot_fail http_fail backend_fail worker_fail disk_warn import_fail threshold_fail first_failure last_failure duration cpu ram
  start="$(metadata_value "$dir" START_TIME)"; expected="$(metadata_value "$dir" EXPECTED_END_TIME)"
  sha="$(metadata_value "$dir" RELEASE_SHA)"; duration="$(metadata_value "$dir" DURATION_HOURS)"
  cpu="$(metadata_value "$dir" CPU_COUNT)"; ram="$(metadata_value "$dir" TOTAL_RAM)"
  current="$(git -C "$ROOT/current" rev-parse --short=12 HEAD 2>/dev/null || echo unknown)"; end="$(utc_now)"
  ...
  full_warn="$(grep -rl '^RESULT=WARNING$' "$dir/samples" 2>/dev/null | wc -l | tr -d ' ' || true)"
  ...
  maintenance="$(grep -rl '^RESULT=MAINTENANCE$' "$dir/samples" 2>/dev/null | wc -l | tr -d ' ' || true)"
  snapshot_fail="$(grep -rl '^IMPORT_SNAPSHOT_RETURN_CODE=[^0]' "$dir/samples" 2>/dev/null | wc -l | tr -d ' ' || true)"
  http_fail="$(grep -Ec 'HTTP=(000|4[0-9][0-9]|5[0-9][0-9])' "$dir/live.log" 2>/dev/null || true)"
  backend_fail="$(grep -c 'BACKEND=FAIL' "$dir/live.log" 2>/dev/null || true)"
  worker_fail="$(grep -c 'WORKER=FAIL' "$dir/live.log" 2>/dev/null || true)"
  disk_warn="$(grep -Rci 'disk usage\|/var/lib/docker=.*%\|/srv/112233.es/.*=%' "$dir/samples" 2>/dev/null || true)"
  import_fail="$(grep -Rh '^IMPORT_SOURCE|' "$dir/samples" 2>/dev/null | awk -F'|' '$3 != "success" {n++} END {print n+0}')"
  threshold_fail="$(grep -Rci 'healthy sources.*required\|healthy-source threshold' "$dir/samples" 2>/dev/null || true)"
  ...
  final=PASS
  [[ "$full_fail" == 0 && "$snapshot_fail" == 0 && "$http_fail" == 0 && "$backend_fail" == 0 && "$worker_fail" == 0 && "$current" == "$sha" ]] || final=FAIL
  if [[ "$final" == PASS && ( "$full_warn" != 0 || "$skipped_pressure" != 0 || "$skipped_running" != 0 || "$maintenance" != 0 ) ]]; then
    final=PASS_WITH_WARNINGS
  fi

  {
    printf '# Production acceptance report\n\n'
    kv START "$start"; kv END "$end"; kv EXPECTED_END "$expected"; kv DURATION_HOURS "$duration"
    kv RELEASE_SHA "$sha"; kv RELEASE_CHANGED "$([[ "$current" == "$sha" ]] && echo NO || echo YES)"
    kv CPU_COUNT "$cpu"; kv TOTAL_RAM "$ram"; kv LIGHT_INTERVAL "$LIGHT_INTERVAL_SECONDS"; kv FULL_INTERVAL "$FULL_INTERVAL_SECONDS"
    kv TOTAL_LIGHT_CHECKS "$(wc -l < "$dir/live.log" 2>/dev/null || echo 0)"; kv TOTAL_FULL_CHECKS "$total_full"
    kv PASSED_FULL_CHECKS "$full_pass"; kv WARNING_FULL_CHECKS "$full_warn"; kv FAILED_FULL_CHECKS "$full_fail"
    kv SKIPPED_RESOURCE_PRESSURE "$skipped_pressure"; kv SKIPPED_PREVIOUS_CHECK_RUNNING "$skipped_running"; kv MAINTENANCE_SAMPLES "$maintenance"
    kv IMPORT_SNAPSHOT_FAILURES "$snapshot_fail"; kv HTTP_FAILURES "$http_fail"; kv BACKEND_FAILURES "$backend_fail"; kv WORKER_FAILURES "$worker_fail"
    kv DISK_WARNINGS "$disk_warn"; kv IMPORT_NON_SUCCESS_SOURCE_ROWS "$import_fail"; kv THRESHOLD_FAILURES "$threshold_fail"
...
start() {
  ...
  if [[ "$requested" == burn-in ]]; then
    ...
    hours="$requested"; [[ "$hours" =~ ^[1-9][0-9]*$ ]] || die "duration hours must be a positive integer"; seconds=$((hours * 3600))
  fi
  [[ -r "$ENV_FILE" ]] || die "production environment file is not readable"
  ...
  run_id="production-acceptance-$(date -u +%Y%m%d-%H%M%S)"; dir="$BASE_DIR/$run_id"
  mkdir -p "$dir/samples"; chmod 700 "$dir" "$dir/samples"
  cpu="$(nproc)"; ram="$(free -h | awk '/^Mem:/ {print $2}')"; swap="$(free -h | awk '/^Swap:/ {print $2}')"
  disk_total="$(df -h "$ROOT" | awk 'NR==2 {print $2}')"; disk_free="$(df -h "$ROOT" | awk 'NR==2 {print $4}')"; load="$(awk '{print $1}' /proc/loadavg)"
  sha="$(git -C "$ROOT/current" rev-parse --short=12 HEAD)"; start_epoch="$(epoch_now)"; end_epoch=$((start_epoch + seconds))
  {
    kv RUN_ID "$run_id"; kv START_TIME "$(utc_now)"; kv START_EPOCH "$start_epoch"; kv EXPECTED_END_TIME "$(date -u -d "@$end_epoch" +%FT%TZ)"; kv END_EPOCH "$end_epoch"
    kv DURATION_HOURS "$hours"; kv LIGHT_INTERVAL_SECONDS "$LIGHT_INTERVAL_SECONDS"; kv FULL_INTERVAL_SECONDS "$FULL_INTERVAL_SECONDS"
    kv RELEASE_SHA "$sha"; kv RELEASE_PATH "$(readlink -f "$ROOT/current")"; kv HOSTNAME "$(hostname)"; kv CPU_COUNT "$cpu"; kv TOTAL_RAM "$ram"; kv SWAP "$swap"
    kv DISK_TOTAL "$disk_total"; kv DISK_FREE_AT_START "$disk_free"; kv BASELINE_LOAD "$load"
  } > "$dir/metadata.txt"
  ...
  systemd-run --quiet --unit="$unit" --property=Type=exec --property=Nice=10 \
    --property=IOSchedulingClass=best-effort --property=IOSchedulingPriority=7 \
    --property=CPUWeight=1 --property=IOWeight=1 \
...
status() {
  ...
  dir="$(active_dir)"; [[ -n "$dir" && -d "$dir" ]] || die "no acceptance run exists"
  unit="ttest-production-acceptance-${dir##*production-acceptance-}"; unit_state="$(systemctl is-active "$unit" 2>/dev/null || true)"
  now="$(epoch_now)"; start_epoch="$(metadata_value "$dir" START_EPOCH)"; end_epoch="$(metadata_value "$dir" END_EPOCH)"
  elapsed=$((now - start_epoch)); remaining=$((end_epoch - now)); (( remaining < 0 )) && remaining=0
  cat "$dir/status"
  printf 'RUN_ID=%s\nUNIT=%s\nUNIT_STATE=%s\nELAPSED=%s\nREMAINING=%s\nEVIDENCE_DIR=%s\n' \
    "$(basename "$dir")" "$unit" "${unit_state:-unknown}" "$(format_duration "$elapsed")" "$(format_duration "$remaining")" "$dir"
  tail -n 1 "$dir/live.log" 2>/dev/null || true
  tail -n 1 "$dir/monitor.log" 2>/dev/null || true
}
